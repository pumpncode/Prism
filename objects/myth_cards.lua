...
SMODS.Consumable({
    ...
    use = function(self, card, area, copier)
        if pseudorandom(pseudoseed('eggo_roll')) < (G.GAME.probabilities.normal / card.ability.odds) then
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
...
G.PRISM.FUNCS.find_card = function(entered_text)
	...
	for i, v in pairs(G.P_CENTERS) do
		if v.set and v.set == "Joker"
        and v.rarity ~= 4
        and v.rarity ~= "pridark_prismatic"
        and v.rarity ~= "cry_exotic"
        and clean_string(entered_text) == clean_string(localize({ type = "name_text", set = "Joker", key = i })) then
			return i
		end
	end
    return nil
